...
class AutoPatchTransform extends Transform implements Plugin<Project> {
    ...
    def initConfig() {
        ...
        Config.methodMap.clear()
        Config.methodMap.putAll(JavaUtils.getMapFromZippedFile(project.projectDir.path + Constants.METHOD_MAP_PATH))
    }
    ...
    void transform(Context context, Collection<TransformInput> inputs, Collection<TransformInput> referencedInputs, TransformOutputProvider outputProvider, boolean isIncremental) throws IOException, TransformException, InterruptedException {
        ...
        box.forEach({ ct ->
            logger.quiet "ct class: ${ct.toString()}"
        })
        autoPatch(box)
//        JavaUtils.removeJarFromLibs()
    ...
    def executeCommand(String commond) {
        ...
        println Config.robustGenerateDirectory
    ...
    def createControlClass(String patchPath, CtClass modifiedClass) {
        CtClass controlClass = PatchesControlFactory.createPatchesControl(modifiedClass, logger);
