...
import com.meituan.robust.annotaion.Add
...
class ReadAnnotation {
    ...
    public static void readAnnotation(List<CtClass> box, Logger log) {
        ...
        synchronized (AutoPatchTransform.class) {
            if (Constants.ModifyAnnotationClass == null) {
                log.quiet(box.get(0).getClassPool().get(Constants.MODIFY_ANNOTATION).methods.toString())
    ...
    public static Set addPatchMethodAndModifiedClass(Set patchMethodSignureSet, CtMethod method) {
        if (Config.methodMap.get(method.longName) == null) {
            print("addPatchMethodAndModifiedClass pint methodmap :" + method.longName);
            JavaUtils.printMap(Config.methodMap);

            if (Config.methodMap == null) {
                print("Config.methodMap == null")
            } else {
                for (String key : Config.methodMap.keySet())
                    print("key is   " + key + "  value is    " + Config.methodMap.get(key));
                print("Config.methodMap != null")
            }
