...
public class SmaliTool {
  ...
  public void dealObscureInSmali() {
    File diretory =
        new File(
            Config.robustGenerateDirectory
                + "classout"
                + File.separator
                + Config.patchPackageName.replaceAll(
    ...
    List<File> smaliFileList =
        covertPathToFile(
            Config.robustGenerateDirectory + "classout" + File.separator,
    ...
    for (File file : smaliFileList) {
      ...
      try {
        ...
        while ((line = reader.readLine()) != null) {
          // 显示行号
          fileContent.append(
              dealWithSmaliLine(line, JavaUtils.getFullClassNameFromFile(file.getPath())) + "\n");
          lineNo++;
        }
  ...
  private String dealWithSmaliLine(final String line, String fullClassName) {
          ...
          if (superClassMapping != null) {
            for (String key : superClassMapping.getMemberMapping().keySet()) {
              ...
              if (!newClassMapping.getMemberMapping().containsKey(key)) {
                newClassMapping
                    .getMemberMapping()
                    .put(key, superClassMapping.getMemberMapping().get(key));
              }
            }
          }
        }
    ...
    for (int index = 0; packageNameList != null && index < packageNameList.size(); index++) {
      ...
      if (result.indexOf(packageNameList.get(index))
          != result.lastIndexOf(packageNameList.get(index))) {
      ...
      // invoke-virtual {v0, v5, v6, p0},
      // Landroid/support/v4/app/LoaderManager;->initLoader(ILandroid/os/Bundle;Landroid/support/v4/app/bi;)Landroid/support/v4/content/Loader;
      if (result.contains("invoke")
          && (packageNameIndex + packageNameList.get(index).length() + 3 < result.length())
          && result
              .substring(
                  packageNameIndex + packageNameList.get(index).length() + 1,
        ...
        // 字段处理
        // sget-object v4,
        // Lcom/sankuai/meituan/fingerprint/FingerprintConfig;->accelerometerInfoList:Ljava/util/List;
        String fieldName =
            result.substring(
                packageNameIndex + packageNameList.get(index).length() + 3,
                result.lastIndexOf(":"));
    ...
    for (int index = 0; packageNameList != null && index < packageNameList.size(); index++) {
      result =
          result.replace(
              packageNameList.get(index), getObscuredClassName(packageNameList.get(index)));
    }
    return result;
  }
  ...
  private String invokeSuperMethodInSmali(final String line, String fullClassName) {
    ...
    String result = line;
    String returnType;
    List<CtMethod> invokeSuperMethodList =
        invokeSuperMethodMap.get(NameManger.getInstance().getPatchNameMap().get(fullClassName));
    if (isInStaticRobustMethod && line.contains(Constants.SMALI_INVOKE_VIRTUAL_COMMAND)) {
      for (CtMethod ctMethod : invokeSuperMethodList) {
        ...
        if ((ctMethod.getName().replaceAll("\\.", "/")
                + ctMethod.getSignature().subSequence(0, ctMethod.getSignature().indexOf(")") + 1))
            .equals(getMethodSignureInSmaliLine(line))) {
          result =
              line.replace(
                  Constants.SMALI_INVOKE_VIRTUAL_COMMAND, Constants.SMALI_INVOKE_SUPER_COMMAND);
          try {
            if (!ctMethod.getReturnType().isPrimitive()) {
              returnType = "L" + ctMethod.getReturnType().getName().replaceAll("\\.", "/");
            } else {
              returnType =
                  String.valueOf(((CtPrimitiveType) ctMethod.getReturnType()).getDescriptor());
            }
            ...
            result =
                result.replace(
                    result.substring(
                        result.indexOf(PACKNAME_START) + 1, result.indexOf(PACKNAME_END)),
  ...
  private String getObscuredMethodSignure(final String line, String className) {
    ...
    if (className.endsWith(Constants.PATCH_SUFFIX)
        && Config.modifiedClassNameList.contains(
            className.substring(0, className.indexOf(Constants.PATCH_SUFFIX)))) {
    ...
    for (int index = line.indexOf("(") + 1; index < endIndex; index++) {
      if (Constants.PACKNAME_START.equals(String.valueOf(methodSigure.charAt(index)))
        ...
        methodSignureBuilder.append(
            methodSigure
                .substring(index + 1, methodSigure.indexOf(Constants.PACKNAME_END, index))
                .replaceAll("/", "\\."));
        if (isArray) {
          methodSignureBuilder.append("[]");
          isArray = false;
    ...
    if (String.valueOf(methodSignureBuilder.charAt(methodSignureBuilder.toString().length() - 1))
        .equals(","))
    ...
    String obscuredMethodName =
        getObscuredMemberName(
            className,
            ReadMapping.getInstance()
                .getMethodSignureWithReturnType(returnTypeList.get(0), obscuredMethodSignure));
    obscuredMethodSignure = obscuredMethodName + parameter;
    //        System.out.println("getObscuredMethodSignure is "+obscuredMethodSignure.substring(0,
    // obscuredMethodSignure.indexOf("(")) + parameter);
  ...
  private List<String> gePackageNameFromSmaliLine(String smaliLine) {
    ...
    for (int index = 0; index < smaliLine.length(); index++) {
      if (Constants.PACKNAME_START.equals(String.valueOf(smaliLine.charAt(index)))
        ...
        packageNameList.add(
            smaliLine
                .substring(index + 1, smaliLine.indexOf(Constants.PACKNAME_END, index))
                .replaceAll("/", "\\."));
  ...
  private String getObscuredMemberName(String className, String memberName) {
    ...
    if (classMapping == null) {
      System.out.println(
          "Warning: getObscuredMemberName  class  name "
              + className
              + "   member name is  "
              + memberName
